...
**Lifecycle status reverse-integration gap.** The mandate requires PDPs to report four statuses back to the DGFiP: Deposited, Rejected, Refused, and Paid. Standard PDP procurement focuses on outbound invoice transmission. At go-live, groups discover the reverse flow requires additional integration work not covered in the PDP contract - and that PDP pricing for high-frequency status-message API calls was not included in the initial cost model.
